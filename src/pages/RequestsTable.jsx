...
//   const { account = "", amount = 0, requiredDate, pickupAddress = "", deliveryAddress = "", bankEntity = "", beneficiaryId = "" } = paymentInstructions;
...
//         let snapshot;
//         if (role === "client") {
//           const q = query(collection(db, "serviceRequests"), where("userId", "==", currentUserId));
...
//       <div className="mt-42 overflow-x-auto border rounded-lg"> {/* Ajusta mt-20 según la altura de los filtros */}
...
//                     )}
//                   </td>
//                   <td className="border px-4 py-2">{formatPaymentInstructions(req.paymentInstructions)}</td>
//                   <td className="border px-4 py-2">
//                     {req.attachment1 && (
//                       <a
...
// ========= Helpers (mismos formatos que tu versión original) =========
const formatDate = (dateInput) => {
  let date;
  if (dateInput?.seconds) {
    date = new Date(dateInput.seconds * 1000);
...
export const RequestsTable = () => {
  const [requests, setRequests] = useState([]);
  const [filters, setFilters] = useState({});
  const [providers, setProviders] = useState([]); // lista de proveedores para el admin
    ...
    const fetchRequests = async () => {
      try {
        ...
        if (role === "client") {
          ...
          // El proveedor solo ve lo que se le asignó
          const q = query(
            collection(db, "serviceRequests"),
        ...
        // mapa de compañías de los dueños de la solicitud
              ...
              filteredRequests.map((req) => (
                <tr key={req.id} className="hover:bg-gray-50">
                  ...
                  <td className="border px-4 py-2">
                    ...
                    {/* proveedor solo puede subir si está asignado a esa fila */}
                    {role === "provider" && isProviderAssignedToRow(req) && (
                      <input
                        type="file"
                        onChange={(e) => handleUploadAttachment2(req.id, e.target.files[0])}
                        className="mt-2"
